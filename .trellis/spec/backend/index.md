...
## Module Map
- `api/`: lifecycle entrypoints and flow orchestration.
- `source/data_manager/`: config parsing, DB wrappers, CTS domain objects.
- `source/solver/`: clustering, tree-building, timing propagation.
- `source/module/`: reusable services such as router and evaluator.
...
- Confirm the nearest `CMakeLists.txt`, tests, and interface callers were updated when required.
