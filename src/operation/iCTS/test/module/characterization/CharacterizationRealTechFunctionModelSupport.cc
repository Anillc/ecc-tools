...
// ***************************************************************************************
/**
 * @file CharacterizationRealTechFunctionModelSupport.cc
    ...
    -> std::optional<std::vector<std::string>>
{
  ...
  return unit_keys;
                              ...
                              double max_slew_ns, double max_cap_pf) -> FunctionalComposePrediction
                              {
                                ...
                                for (unsigned iteration = 1U; iteration <= max_iterations; ++iteration) {
                                  ...
                                  for (std::size_t unit_index = 0U; unit_index < unit_count; ++unit_index) {
                                    const auto response = EvalFunctionalSurfaceModel(*unit_models.at(unit_index), slews.at(unit_index),
                                                                                     load_caps.at(unit_index + 1U));
                                  ...
                                  for (std::size_t reverse_index = 0U; reverse_index < unit_count; ++reverse_index) {
                                    ...
                                    const auto response = EvalFunctionalSurfaceModel(*unit_models.at(unit_index), slews.at(unit_index),
                                                                                     next_load_caps.at(unit_index + 1U));
                                ...
                                for (std::size_t unit_index = 0U; unit_index < unit_count; ++unit_index) {
                                  const auto response = EvalFunctionalSurfaceModel(*unit_models.at(unit_index), slews.at(unit_index),
                                                                                   load_caps.at(unit_index + 1U));
                                  if (!std::isfinite(response.output_slew_ns) || !std::isfinite(response.driven_cap_pf)
                                      || !std::isfinite(response.delay_ns) || !std::isfinite(response.power_w)
                                ...
                                prediction.is_valid = IsFinitePositive(prediction.output_slew_ns)
                                                      && IsFinitePositive(prediction.driven_cap_pf) && std::isfinite(prediction.delay_ns)
                                                      && prediction.delay_ns >= 0.0 && std::isfinite(prediction.power_w)
                              ...
                              auto PredictStructuralCapFunctionalCompose(const std::vector<const FunctionalSurfaceModel*>& unit_models,
                                                                         const std::vector<const StructuralCapOperator*>& cap_operators,
                                                                         double input_slew_ns, double load_cap_pf, double max_slew_ns,
                                                                         double max_cap_pf) -> FunctionalComposePrediction
                              {
                                FunctionalComposePrediction prediction;
                                const std::size_t unit_count = unit_models.size();
                                if (unit_count == 0U || unit_count != cap_operators.size() || !IsFinitePositive(input_slew_ns)
                                    || !IsFinitePositive(load_cap_pf)) {
                                  return prediction;
                                }
                                ...
                                for (std::size_t unit_index = 0U; unit_index < unit_count; ++unit_index) {
                                  const auto response = EvalFunctionalSurfaceModel(*unit_models.at(unit_index), slews.at(unit_index),
                                                                                   load_caps.at(unit_index + 1U));
                                  if (!std::isfinite(response.output_slew_ns) || !std::isfinite(response.delay_ns)
