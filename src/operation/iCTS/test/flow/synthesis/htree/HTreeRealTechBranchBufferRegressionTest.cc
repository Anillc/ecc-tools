...
TEST(HTreeRealTechSmokeTest, CallerFacingTopBoundaryBuildOptionsPropagateWhenFeasible)
{
  const auto& setup_state = common_realtech::EnsureRealTechSetup();
  if (setup_state.mode != common_realtech::RealTechMode::kRealTech || !setup_state.setup_succeeded) {
    GTEST_SKIP() << setup_state.summary;
  ...
  const auto baseline_result = icts::HTree::build(baseline_root_net);
  ...
  auto top_boundary_result = icts::HTree::build(top_boundary_root_net, icts::HTree::BuildOptions{
  ...
  auto impossible_top_boundary_result = icts::HTree::build(impossible_root_net, icts::HTree::BuildOptions{
                                                                                    .force_branch_buffer = std::nullopt,
