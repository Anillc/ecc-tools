...
auto MakeToleranceSkipResult(const std::string& reason) -> ClockSynthesisToleranceComparisonResult
{
  ClockSynthesisToleranceComparisonResult result;
  result.skipped = true;
  result.skip_reason = reason;
  return result;
}
...
auto CalcLeafLoadDistances(const icts::HTreeBuilder::BuildResult& htree_result, const std::vector<icts::Pin*>& sinks) -> std::vector<double>
{
...
auto PopulateDistanceDeltaStats(const std::vector<double>& default_distances, const std::vector<double>& legacy_distances,
                                ClockSynthesisToleranceComparisonResult& comparison_result) -> void
{
...
auto BuildRouteTimingForNet(const icts::Net* net) -> NetRouteTimingSummary
{
  ...
  auto rc_tree = icts::Router::buildRCTree(route_tree);
...
auto CalcClockTreeTimingSummary(const icts::ClockSynthesis::BuildResult& result, const std::vector<icts::Pin*>& sinks)
    -> ClockTreeTimingSummary
{
  ClockTreeTimingSummary summary;
...
auto AppendToleranceCaseFailures(const ClockSynthesisToleranceComparisonRecord& record, std::vector<std::string>& failure_messages) -> void
{
...
auto EvaluateArm9FullSinkTopologyToleranceComparison() -> ClockSynthesisToleranceComparisonResult
{
  ...
  realtech_support::RealTechCharSession char_session;
  if (const auto prepare_error = char_session.prepare("clock_synthesis_arm9_full_sink_topology_tolerance", std::nullopt,
                                                      kSynthesisSmokeMaxSlewNs, kSynthesisSmokeMaxCapPf, true);
      prepare_error.has_value()) {
    return MakeToleranceSkipResult(*prepare_error);
  }

  ClockSynthesisToleranceComparisonResult comparison_result;
  ...
  for (const double topology_tolerance : {0.1, 0.0}) {
    ...
    const auto timing_summary = CalcClockTreeTimingSummary(result, selected_clock_data.sinks);

    ClockSynthesisToleranceComparisonRecord record{
  ...
  comparison_result.report_written
      = WriteClockSynthesisMatrixReport("clock_synthesis_arm9_full_sink_topology_tolerance", "topology_tolerance_comparison.csv",
                                        FormatClockSynthesisToleranceComparisonReport("clock_synthesis_arm9_full_sink_topology_tolerance",
                                                                                      selected_clock_data, comparison_result));
