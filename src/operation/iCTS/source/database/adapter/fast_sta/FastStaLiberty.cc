 ...
 * @date 2026-05-18
 * @brief Fast STA Liberty data snapshot implementation.
 */
...
#include "adapter/sta/STAAdapterInternal.hh"
...
auto convertTableAxisValue(ista::LibCell* lib_cell, ista::LibLutTableTemplate::Variable variable, double value) -> double
{
  ...
  switch (toFastStaAxisKind(variable)) {
    ...
    case FastStaLibertyAxisKind::kOutputLoad:
      return sta_adapter_internal::ConvertLibCapToPf(lib_cell, value);
...
auto convertTableValue(ista::LibCell* lib_cell, FastStaLibertyTableKind kind, double value) -> double
{
  switch (kind) {
    ...
    case FastStaLibertyTableKind::kOutputSlew:
      return sta_adapter_internal::ConvertLibTimeToNs(lib_cell, value);
...
auto appendAxis(ista::LibCell* lib_cell, ista::LibTable* table, std::optional<ista::LibLutTableTemplate::Variable> variable,
                unsigned axis_index, FastStaLibertyTable& snapshot) -> void
{
  ...
  snapshot.axes.push_back(std::move(axis));
}

auto snapshotTable(ista::LibCell* lib_cell, ista::LibTable* table, FastStaLibertyTableKind kind, FastStaTransition transition)
    -> FastStaLibertyTable
{
  FastStaLibertyTable snapshot;
  snapshot.kind = kind;
  ...
  if (table_template != nullptr) {
    appendAxis(lib_cell, table, table_template->get_template_variable1(), 0U, snapshot);
    appendAxis(lib_cell, table, table_template->get_template_variable2(), 1U, snapshot);
  }

  const auto& source_values = table->get_table_values();
  snapshot.values.reserve(source_values.size());
  for (const auto& value : source_values) {
    if (value == nullptr) {
      continue;
    }
    snapshot.values.push_back(convertTableValue(lib_cell, kind, value->getFloatValue()));
  }
  return snapshot;
}

auto snapshotDelayTable(ista::LibCell* lib_cell, ista::LibDelayTableModel* delay_model, ista::LibTable::TableType table_type,
                        FastStaLibertyTableKind kind, FastStaTransition transition, std::vector<FastStaLibertyTable>& tables) -> void
{
  ...
  tables.push_back(snapshotTable(lib_cell, table, kind, transition));
}

auto snapshotPowerTable(ista::LibCell* lib_cell, ista::LibPowerTableModel* power_model, ista::LibTable::TableType table_type,
                        FastStaTransition transition, std::vector<FastStaLibertyTable>& tables) -> void
{
  ...
  tables.push_back(snapshotTable(lib_cell, table, FastStaLibertyTableKind::kInternalPower, transition));
}
...
auto snapshotPowerArcTables(ista::LibCell* lib_cell, ista::LibPowerArcSet* power_arc_set, FastStaLibertyArc& snapshot_arc) -> void
{
  ...
  for (const auto& power_arc_holder : power_arc_set->get_power_arcs()) {
    ...
    snapshotPowerTable(lib_cell, power_model, ista::LibTable::TableType::kRisePower, FastStaTransition::kRise,
                       snapshot_arc.internal_power_tables);
    snapshotPowerTable(lib_cell, power_model, ista::LibTable::TableType::kFallPower, FastStaTransition::kFall,
                       snapshot_arc.internal_power_tables);
  }
}

auto snapshotBufferCellFromLibCell(ista::LibCell* lib_cell) -> FastStaLibertyCell
{
  if (lib_cell == nullptr) {
  ...
  if (output_port != nullptr) {
    if (auto cap_limit = output_port->get_port_cap_limit(ista::AnalysisMode::kMax); cap_limit.has_value()) {
      output_cap_limit_pf = sta_adapter_internal::ConvertLibCapToPf(lib_cell, *cap_limit);
    }
  }
  ...
  if (input_port != nullptr) {
    if (auto slew_limit = input_port->get_port_slew_limit(ista::AnalysisMode::kMax); slew_limit.has_value()) {
      input_slew_limit_ns = sta_adapter_internal::ConvertLibTimeToNs(lib_cell, *slew_limit);
    }
  }
  ...
  FastStaLibertyCell cell{
      ...
      .input_cap_pf = sta_adapter_internal::QueryLibPortCapacitancePf(lib_cell, input_port),
  ...
  auto timing_arc_set = sta_adapter_internal::FindBufferArcSet(lib_cell);
  ...
  if (delay_model != nullptr) {
    snapshotDelayTable(lib_cell, delay_model, ista::LibTable::TableType::kCellRise, FastStaLibertyTableKind::kCellDelay,
                       FastStaTransition::kRise, cell.timing_arc.delay_tables);
    snapshotDelayTable(lib_cell, delay_model, ista::LibTable::TableType::kCellFall, FastStaLibertyTableKind::kCellDelay,
                       FastStaTransition::kFall, cell.timing_arc.delay_tables);
    snapshotDelayTable(lib_cell, delay_model, ista::LibTable::TableType::kRiseTransition, FastStaLibertyTableKind::kOutputSlew,
                       FastStaTransition::kRise, cell.timing_arc.slew_tables);
    snapshotDelayTable(lib_cell, delay_model, ista::LibTable::TableType::kFallTransition, FastStaLibertyTableKind::kOutputSlew,
  ...
  snapshotPowerArcTables(lib_cell, power_arc_set.value_or(nullptr), cell.timing_arc);
  return cell;
}

}  // namespace

auto FastStaLiberty::snapshotBufferCell(const std::string& cell_master) -> FastStaLibertyCell
{
  auto* lib_cell = sta_adapter_internal::FindLibertyCellByMaster(cell_master);
  if (lib_cell != nullptr) {
    return snapshotBufferCellFromLibCell(lib_cell);
  }
