...
auto STAAdapter::queryCharInputPinCap(const std::string& cell_master) -> double
{
  ...
  return sta_adapter_timing_query::ConvertLibCapToPf(lib_cell, input->get_port_cap());
}
...
auto STAAdapter::queryPinCapacitance(const Pin* pin) -> double
{
  ...
  return sta_adapter_timing_query::QueryLibPortCapacitancePf(lib_cell, lib_port);
}
