...
auto clearSummary(ClockTreeSummary& summary) -> void
{
  ...
  summary.total_clock_tree_wirelength_um = 0.0;
  summary.max_clock_net_wirelength_dbu = 0;
  summary.total_clock_tree_wirelength_dbu = 0.0;
...
auto syncCompatibilityAliases(ClockTreeSummary& summary) -> void
{
...
auto dbuToUm(int64_t dbu, const ClockTreeSummary& summary) -> double
{
...
auto accumulateInstStatistics(const Inst& inst, CTSStatistics& statistics) -> void
{
  if (!inst.is_buffer()) {
    return;
  }
...
auto appendClockNetStatistics(const std::vector<ClockNetMeasurement>& measurements, ClockTreeSummary& summary, CTSStatistics& statistics)
    -> void
{
  for (const auto& measurement : measurements) {
    ...
    summary.total_clock_tree_wirelength_um += wirelength_um;
    summary.max_clock_net_wirelength_um = std::max(summary.max_clock_net_wirelength_um, wirelength_um);
    summary.total_clock_tree_wirelength_dbu += static_cast<double>(wirelength);
...
auto appendClockTimings(bool query_sta_timing, ClockTreeSummary& summary) -> void
{
  if (!query_sta_timing) {
...
auto appendClockLatencySkew(ClockTreeSummary& summary) -> void
{
  ...
  for (const auto& metric : latency_skew_metrics) {
    summary.clocks_latency_skew.push_back(ClockTreeSummary::ClockLatencySkew{
...
auto emitClockTimingTables(const ClockTreeSummary& summary) -> void
{
  if (!summary.clocks_timing.empty()) {
    ...
    schema::EmitTable("CTS Clock Timing Summary",
  ...
  if (!summary.clocks_latency_skew.empty()) {
    ...
    schema::EmitTable("CTS Clock Latency Skew Summary",
