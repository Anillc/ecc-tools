...
auto buildWirelengthReportRows(const CTSStatistics& statistics) -> schema::TableRows
{
  return {
...
auto buildCellStatsReportRows(const CTSStatistics& statistics) -> schema::TableRows
{
...
auto buildLibCellDistReportRows(const CTSStatistics& statistics) -> schema::TableRows
{
  schema::TableRows rows;
                     ...
                     const schema::TableRows& rows) -> bool
{
  ...
  if (!stream.is_open()) {
    LOG_WARNING << "CTSStatisticsWriter: failed to open statistics report " << path.string();
    return false;
  }
...
auto CTSStatisticsWriter::writeReports(const std::filesystem::path& root_dir, const CTSStatistics& statistics) -> bool
{
  ...
  if (error_code) {
    LOG_WARNING << "CTSStatisticsWriter: failed to create statistics directory " << statistics_dir.string();
    return false;
  }
...
auto CTSStatisticsWriter::emitLogTables(const CTSStatistics& statistics) -> void
{
