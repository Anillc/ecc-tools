...
// ***************************************************************************************
/**
 * @file CTSGdsWriter.cc
 * @author Dawn Li (dawnli619215645@gmail.com)
 * @date 2026-04-28
 * @brief Minimal binary GDSII stream writer implementation for CTS GDS report files.
 */
...
auto ensureParentDirectory(const std::filesystem::path& path) -> bool
{
  ...
  if (error) {
    LOG_WARNING << "CTSGdsWriter: failed to create directory " << parent_path.string() << ": " << error.message();
    return false;
  }
  return true;
}
...
auto makeGdsReal8(double value) -> std::array<uint8_t, 8>
{
  ...
  if (exponent <= 0 || exponent >= 128) {
    LOG_WARNING << "CTSGdsWriter: real8 value is outside GDSII exponent range.";
    return bytes;
  }
...
auto writeRecord(std::ofstream& stream, GdsRecord record, GdsDataType data_type, const std::vector<uint8_t>& payload) -> bool
{
  ...
  if (length > static_cast<std::size_t>(std::numeric_limits<uint16_t>::max())) {
    LOG_WARNING << "CTSGdsWriter: record payload is too large for one GDSII record.";
    return false;
  }
...
auto writeLayerDatatype(std::ofstream& stream, const CTSGdsLayerKey& key, bool text) -> bool
{
...
auto writePath(std::ofstream& stream, const CTSGdsPath& path) -> bool
{
...
auto writeBoundary(std::ofstream& stream, const CTSGdsBoundary& boundary) -> bool
{
...
auto writeText(std::ofstream& stream, const CTSGdsText& text) -> bool
{
  if (text.text.empty()) {
    return true;
  }
...
auto CTSGdsWriter::writeBinary(const std::filesystem::path& path, const CTSGdsLibrary& library) -> bool
{
  ...
  if (!stream.is_open()) {
    LOG_WARNING << "CTSGdsWriter: failed to open " << path.string() << " for binary GDSII output.";
    return false;
  }
...
auto CTSGdsWriter::writeLayerProperties(const std::filesystem::path& path, const std::vector<CTSGdsLayerProperty>& layers) -> bool
{
  if (!ensureParentDirectory(path)) {
    return false;
  }
  ...
  if (!stream.is_open()) {
    LOG_WARNING << "CTSGdsWriter: failed to open " << path.string() << " for layer properties.";
    return false;
  }
