...
auto CreateBufferInstance(SegmentBuilder::BuildResult& result, const std::string& inst_name, const std::string& cell_master,
...
auto MaterializeSegment(SegmentBuilder::BuildResult& result, Net& source_net, Pin* source, Pin* sink, const BufferingPattern& pattern,
                        const SegmentBuilder::BuildOptions& options) -> bool
{
  ...
  for (std::size_t buffer_index = 0; buffer_index < buffer_count; ++buffer_index) {
    ...
    if (ports == nullptr) {
      LOG_WARNING << "SegmentBuilder: unresolved ports for source-to-root buffer master " << cell_masters.at(buffer_index) << ".";
...
auto EmitSegmentSummary(const SegmentBuilder::BuildResult& result, const SegmentBuilder::BuildOptions& options) -> void
{
  schema::EmitKeyValueTable(
      "SegmentBuilder Build Summary",
      {
...
auto SegmentBuilder::build(Net& source_net, Pin* source, Pin* sink, const BuildOptions& options) -> BuildResult
{
  BuildResult result;
  if (source == nullptr || sink == nullptr) {
    result.failure_reason = "null_source_or_sink_pin";
    LOG_ERROR << "SegmentBuilder: source-to-root build failed because source or sink pin is null.";
  ...
  if (options.required_load_cap_pf <= 0.0) {
    ...
    LOG_ERROR << "SegmentBuilder: source-to-root segment requires a positive root-input load cap.";
    EmitSegmentSummary(result, options);
    return result;
  }
  if (options.source_drive_cap_pf <= 0.0) {
    result.failure_reason = "unresolved_source_drive_cap";
    LOG_ERROR << "SegmentBuilder: source-to-root segment requires a positive source drive cap.";
    EmitSegmentSummary(result, options);
    return result;
  }
  ...
  result.success = MaterializeSegment(result, source_net, source, sink, *selected_pattern, options);
  EmitSegmentSummary(result, options);
  return result;
}
