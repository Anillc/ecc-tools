...
// ***************************************************************************************
/**
 * @file HTreeSynthesisSummary.cc
 * @author Dawn Li (dawnli619215645@gmail.com)
 * @date 2026-04-24
 * @brief H-tree synthesis summary assembly.
 */
                              ...
                              const HTreeTopologyDepthSummary& selected_summary) -> void
{
  ...
  if (!result.best_char.has_value()) {
    LOG_WARNING << "HTreeBuilder: synthesis summary skipped because no selected topology char is available.";
    return;
  }
  ...
  logformat::TableRows synthesis_summary_rows = {
      ...
      {"final_frontier_count", std::to_string(selected_summary.final_frontier_count),
       "selected-depth root frontier size before boundary filtering and sink-load-profile legality filtering"},
      ...
      {"candidate_frontier_entry_count", std::to_string(selected_summary.candidate_frontier_entry_count),
       selected_has_boundary_constraints ? "selected-depth sink-load-profile-legal candidate frontier entries before feasible filtering"
                                         : "not assembled on unrestricted builds"},
      ...
      {"feasible_frontier_entry_count", std::to_string(selected_summary.feasible_frontier_entry_count),
       "selected-depth sink-load-profile-legal frontier entries after feasible filtering"},
  ...
  LogInfoTable("HTreeBuilder Synthesis Summary", {"Item", "Value", "Detail"}, synthesis_summary_rows);
}
