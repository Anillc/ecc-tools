...
// ***************************************************************************************
/**
 * @file SourceToRootSegmentBuilder.cc
...
auto CreateBufferInstance(SourceToRootSegmentBuilder::BuildResult& result, const std::string& inst_name, const std::string& cell_master,
...
auto RecordInsertedInstLevel(SourceToRootSegmentBuilder::BuildResult& result, Inst* inst, int topology_level, std::size_t index_in_level)
    -> void
{
  if (inst == nullptr) {
    return;
...
auto RecordInsertedNetLevel(SourceToRootSegmentBuilder::BuildResult& result, Net* net, int topology_level, std::size_t index_in_level)
    -> void
{
  if (net == nullptr) {
    return;
...
auto CreateNet(SourceToRootSegmentBuilder::BuildResult& result, const std::string& net_name, Pin* driver, const std::vector<Pin*>& loads,
...
auto BuildSourceToRootSegmentObjects(SourceToRootSegmentBuilder::BuildResult& result, Net& source_net, Pin* source, Pin* sink,
                                     const BufferingPattern& pattern, const SourceToRootSegmentBuilder::BuildOptions& options) -> bool
{
  ...
  htree_builder::BufferPortCache port_cache;
  ...
  for (std::size_t buffer_index = 0; buffer_index < buffer_count; ++buffer_index) {
    const auto* ports = port_cache.get(cell_masters.at(buffer_index));
    if (ports == nullptr) {
      LOG_WARNING << "SourceToRootSegmentBuilder: unresolved ports for source-to-root buffer master " << cell_masters.at(buffer_index)
                  << ".";
...
auto EmitSegmentSummary(const SourceToRootSegmentBuilder::BuildResult& result, const SourceToRootSegmentBuilder::BuildOptions& options)
    -> void
{
  schema::EmitKeyValueTable(
      "SourceToRootSegmentBuilder Build Summary",
      {
...
auto ConfigureCharOptions(const std::vector<double>& requested_lengths_um) -> CharBuilder::InitOptions
{
  ...
  const auto char_grid_plan = htree_builder::ResolveCharacterizationGridPlan(requested_lengths_um);
  ...
  auto direct_indices = htree_builder::ResolveDirectCharacterizationLengthIndices(requested_lengths_um, char_grid_plan);
...
auto SourceToRootSegmentBuilder::build(Net& source_net, Pin* source, Pin* sink, const BuildOptions& options) -> BuildResult
{
  BuildResult result;
  if (source == nullptr || sink == nullptr) {
    result.failure_reason = "null_source_or_sink_pin";
    LOG_ERROR << "SourceToRootSegmentBuilder: source-to-root build failed because source or sink pin is null.";
  ...
  if (options.required_load_cap_pf <= 0.0) {
    ...
    LOG_ERROR << "SourceToRootSegmentBuilder: source-to-root segment requires a positive root-input load cap.";
    EmitSegmentSummary(result, options);
    return result;
  }
  if (options.source_drive_cap_pf <= 0.0) {
    result.failure_reason = "unresolved_source_drive_cap";
    LOG_ERROR << "SourceToRootSegmentBuilder: source-to-root segment requires a positive source drive cap.";
    EmitSegmentSummary(result, options);
    return result;
  }
  ...
  result.required_load_cap_idx
      = htree_builder::CoveringBoundaryIndex(options.required_load_cap_pf, char_builder.get_cap_lattice()).value_or(0U);
  result.source_drive_cap_idx
      = htree_builder::CoveringBoundaryIndex(options.source_drive_cap_pf, char_builder.get_cap_lattice()).value_or(0U);
  if (options.min_input_slew_ns.has_value()) {
    result.min_input_slew_idx = htree_builder::CoveringBoundaryIndex(*options.min_input_slew_ns, char_builder.get_slew_lattice());
  }
  ...
  htree_builder::BufferPatternRegistry pattern_registry;
  for (const auto& pattern : char_builder.get_buffering_patterns()) {
    pattern_registry.add(pattern);
  }
  auto entry_sets_by_length
      = htree_builder::SynthesizeSegmentEntrySets(char_builder.get_segment_chars(), pattern_registry, {result.length_idx});
  ...
  const auto* selected_pattern = pattern_registry.find(result.best_char->get_pattern_id());
  if (selected_pattern == nullptr) {
    result.failure_reason = "missing_selected_segment_pattern";
    EmitSegmentSummary(result, options);
    return result;
  }
  result.success = BuildSourceToRootSegmentObjects(result, source_net, source, sink, *selected_pattern, options);
  EmitSegmentSummary(result, options);
  return result;
}
