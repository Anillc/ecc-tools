...
// ***************************************************************************************
/**
 * @file HTreeTopologyDepthSearch.cc
    ...
    -> HTreeTopologyDepthSearchResult
{
  ...
  for (const unsigned depth : depth_candidates) {
    auto candidate_result
        = EvaluateTopologyDepthCandidate(topology, full_level_plans, depth, entry_sets_by_length, segment_pattern_registry,
                                         base_resolved_options, exploration.sink_load_profile_legality_context, char_slew_steps);
