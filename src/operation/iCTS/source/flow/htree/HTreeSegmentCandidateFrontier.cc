...
// ***************************************************************************************
/**
 * @file HTreeSegmentCandidateFrontier.cc
    ...
    -> std::pair<SegmentCandidateFrontierSet, unsigned>
{
  ...
  auto [all_frontier_entries, after_all_pattern_id] = ComposeSegmentCandidateFrontierEntries(
      upstream.all_frontier_entries, downstream.all_frontier_entries, pattern_registry, next_pattern_id);
  ...
  auto [branch_frontier_entries, after_branch_pattern_id] = ComposeSegmentCandidateFrontierEntries(
      upstream.all_frontier_entries, downstream.branch_buffered_entries, pattern_registry, next_pattern_id);
  ...
  auto [leaf_frontier_entries, after_leaf_pattern_id] = ComposeSegmentCandidateFrontierEntries(
      upstream.all_frontier_entries, downstream.leaf_unbuffered_entries, pattern_registry, next_pattern_id);
...
auto BuildBaseSegmentCandidateLengthEntrySets(const std::vector<SegmentChar>& chars, const BufferPatternRegistry& pattern_registry)
  ...
  while (!pending_states.empty()) {
    ...
    for (unsigned left_length_idx = 1U; left_length_idx < target_length_idx; ++left_length_idx) {
      ...
      auto [composed_entry_set, updated_next_pattern_id]
          = ComposeSegmentCandidateFrontierSet(*left_entry_set, *right_entry_set, pattern_registry, next_pattern_id);
...
auto SynthesizeSegmentEntrySets(const std::vector<SegmentChar>& base_segment_chars, BufferPatternRegistry& pattern_registry,
    ...
    -> std::unordered_map<unsigned, SegmentCandidateFrontierSet>
{
  auto entry_sets_by_length = BuildBaseSegmentCandidateLengthEntrySets(base_segment_chars, pattern_registry);
