...
// ***************************************************************************************
/**
 * @file HTreeClockTreeObjectBuilder.cc
...
#include "htree/HTreeClockTopologyBuildContext.hh"
...
auto BuildSegmentObjectsAndGetEntryLoads(HTreeClockTopologyBuildContext& context, const TreeNode& parent_node, const TreeNode& child_node,
...
auto BuildClockTreeObjects(HTreeBuilder::BuildResult& result, const BufferPatternRegistry& segment_pattern_registry) -> void
{
  ...
  if (topology_levels.size() <= 1U || candidate_level_count == 0U || candidate_level_count >= topology_levels.size()) {
    result.failure_reason = "invalid_clock_tree_object_depth";
    return;
  }
  ...
  LOG_FATAL_IF(root_node == nullptr) << "HTreeBuilder: topology root is missing during clock-tree object construction.";

  LOG_FATAL_IF(result.root_net == nullptr) << "HTreeBuilder: input root net is missing during clock-tree object construction.";
  LOG_FATAL_IF(result.root_output_pin == nullptr)
      << "HTreeBuilder: input root net driver pin is missing during clock-tree object construction.";
  result.root_inst = result.root_output_pin->get_inst();
  result.root_input_pin = FindSingleBufferInputPin(result.root_inst);

  HTreeClockTopologyBuildContext context{
      .result = &result,
  ...
  for (std::size_t reverse_depth = candidate_level_count; reverse_depth > 0U; --reverse_depth) {
    ...
    LOG_FATAL_IF(segment_pattern == nullptr)
        << "HTreeBuilder: missing selected segment pattern metadata during clock-tree object construction.";
  ...
  LOG_FATAL_IF(root_entry_loads.empty()) << "HTreeBuilder: root entry loads are empty during clock-tree object construction.";
