...
auto HTreeBuilder::build(const std::vector<Pin*>& loads, const BuildOptions& options) -> BuildResult
{
  ...
  const auto char_flow = RunCharacterizationFlow(result.topology, dbu_per_um, result, char_builder);
  ...
  const auto full_level_plans = BuildLevelPlans(result.topology, char_flow.length_step_um, dbu_per_um);
  ...
  const auto covered_global_feasible_pool
      = FilterGlobalEntriesByActualBoundaryCoverage(exploration.global_feasible_pool, exploration.candidate_evaluations, result.topology,
                                                    segment_pattern_registry, exploration.actual_load_legality_context);
  const auto covered_global_candidate_pool
      = FilterGlobalEntriesByActualBoundaryCoverage(exploration.global_candidate_pool, exploration.candidate_evaluations, result.topology,
                                                    segment_pattern_registry, exploration.actual_load_legality_context);
  ...
  LogHTreeBuildSummary(result, selected_evaluation, selected_summary);
