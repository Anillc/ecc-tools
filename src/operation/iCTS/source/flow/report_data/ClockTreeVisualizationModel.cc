...
auto makeSegment(const ClockTreeReportClock& clock, const ClockTreeReportSegment& segment, ClockTreeReportView view)
...
auto appendReportSegments(const ClockTreeReportData& report_data, ClockTreeVisualizationModel& model) -> void
{
    ...
    for (const auto& net : clock.nets) {
      for (const auto& segment : net.routed_segments) {
        if (isValidLocation(segment.begin) && isValidLocation(segment.end)) {
          model.design_segments.push_back(makeSegment(clock, segment, ClockTreeReportView::kDesign));
        }
      }
      for (const auto& segment : net.flyline_segments) {
        if (isValidLocation(segment.begin) && isValidLocation(segment.end)) {
          model.flyline_segments.push_back(makeSegment(clock, segment, ClockTreeReportView::kFlyline));
        }
      }
    }
  }
...
auto appendReportInsts(const ClockTreeReportData& report_data, ClockTreeVisualizationModel& model) -> void
{
  for (const auto& clock : report_data.get_clocks()) {
                                     ...
                                     std::vector<ClockTreeVisualizationSegment>& segments) -> bool
{
  ...
  for (const auto& edge : route_tree.get_edges()) {
    ...
    segments.push_back(ClockTreeVisualizationSegment{
        ...
        .end = target->location,
        .view = ClockTreeReportView::kDesign,
        .net_role = role,
...
auto appendFallbackPinSegments(const Clock& clock, std::size_t clock_index, const Net& net, CTSNetRole role, ClockTreeReportView view,
                               std::vector<ClockTreeVisualizationSegment>& segments) -> bool
{
  ...
  for (auto* load : net.get_loads()) {
    ...
    segments.push_back(ClockTreeVisualizationSegment{
        ...
        .topology_level = 0,
        .routed = false,
        .fallback = view == ClockTreeReportView::kDesign,
    });
    appended = true;
  }
  return appended;
  ...
  for (std::size_t clock_index = 0U; clock_index < clocks.size(); ++clock_index) {
    ...
    for (const auto* net : collectClockNets(*clock)) {
      ...
      if (!appendFallbackRouteTreeSegments(*clock, clock_index, *net, role, model.design_segments)) {
        const bool appended
            = appendFallbackPinSegments(*clock, clock_index, *net, role, ClockTreeReportView::kDesign, model.design_segments);
        if (appended) {
          LOG_WARNING << "CTS visualization model: design view falls back to driver-to-load segments for net " << net->get_name();
        }
      }
      (void) appendFallbackPinSegments(*clock, clock_index, *net, role, ClockTreeReportView::kFlyline, model.flyline_segments);
    }
  }
}
...
auto ClockTreeVisualizationModelBuilder::build(const ClockTreeReportData& report_data) -> ClockTreeVisualizationModel
{
  ClockTreeVisualizationModel model;
  model.has_clocks = !report_data.get_clocks().empty() || !DESIGN_INST.get_clocks().empty();
  model.dbu_per_um = std::max(report_data.get_design_dbu_per_um(), int32_t{1});
  appendReportSegments(report_data, model);
  appendReportInsts(report_data, model);
