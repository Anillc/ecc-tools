...
#include "synthesis/htree/analytical_solver/AnalyticalSelection.hh"
                             ...
                             unsigned char_slew_steps) -> AnalyticalHTreeAttempt
{
  ...
  for (const unsigned depth : depth_candidates) {
    ...
    analytical_solver::AnalyticalSolverRequest solver_request;
    ...
    auto solver_result = analytical_solver::SolveAnalyticalHTreeCandidates(solver_request);
    ...
    for (auto& candidate : solver_result.candidates) {
      ...
      auto validation = analytical_solver::ValidateAnalyticalCandidate(
          candidate, analytical_solver::AnalyticalValidationRequest{
                         .topology = &topology,
