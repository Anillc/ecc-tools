...
// ***************************************************************************************
/**
 * @file HTreeContracts.hh
 * @author Dawn Li (dawnli619215645@gmail.com)
 * @date 2026-05-24
 * @brief Explicit H-tree input, config, output, and summary contracts.
 */
...
enum class HTreeLoadRole
{
  kSink,
  kLocalBuffer
};

struct HTreeLogContext
{
...
struct HTreeRootDriverCompensationReport
{
  bool enabled = false;
  bool valid = false;
  std::string method;
...
struct HTreeBuild
{
  ...
  HTreeBuild(const HTreeBuild&) = delete;
  auto operator=(const HTreeBuild&) -> HTreeBuild& = delete;

  HTreeBuild(HTreeBuild&& rhs) noexcept = default;
  auto operator=(HTreeBuild&& rhs) noexcept -> HTreeBuild& = default;

  HTreeOutput output;
  HTreeSummary summary;
};

struct HTreeDiagnosticBuild : HTreeBuild
{
  HTreeDiagnosticBuild() = default;

  HTreeDiagnosticBuild(const HTreeDiagnosticBuild&) = delete;
  auto operator=(const HTreeDiagnosticBuild&) -> HTreeDiagnosticBuild& = delete;

  HTreeDiagnosticBuild(HTreeDiagnosticBuild&& rhs) noexcept = default;
  auto operator=(HTreeDiagnosticBuild&& rhs) noexcept -> HTreeDiagnosticBuild& = default;
