...
auto ClockTreeVisualizationLayerPolicy::getLayer(const LayerDescriptor& descriptor) -> ClockTreeGdsLayerKey
{
  ...
  if (_next_layer >= std::numeric_limits<int16_t>::max()) {
    ...
    return ClockTreeGdsLayerKey{.layer = std::numeric_limits<int16_t>::max(), .datatype = 0};
  }

  ClockTreeGdsLayerProperty property{
      .key = ClockTreeGdsLayerKey{.layer = _next_layer++, .datatype = 0},
...
auto ClockTreeVisualizationLayerPolicy::logicCellLayer() -> ClockTreeGdsLayerKey
{
...
auto ClockTreeVisualizationLayerPolicy::instLayer(const ClockTreeVisualizationInst& inst) -> std::optional<ClockTreeGdsLayerKey>
{
  switch (inst.role) {
...
auto ClockTreeVisualizationLayerPolicy::segmentLayer(const ClockTreeVisualizationSegment& segment) -> ClockTreeGdsLayerKey
{
  ...
  const bool flyline = segment.view == ClockTreeViewMode::kFlyline;
...
auto ClockTreeVisualizationLayerPolicy::getLayerProperties() const -> std::vector<ClockTreeGdsLayerProperty>
{
  std::vector<ClockTreeGdsLayerProperty> properties;
