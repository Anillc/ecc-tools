...
// ***************************************************************************************
/**
 * @file ClockTreeGdsVisualization.cc
...
#include "visualization/ClockTreeVisualizationLayerPolicy.hh"
...
auto makeBoundary(const ClockTreeGdsLayerKey& key, const Point<int>& origin, int32_t width, int32_t height) -> ClockTreeGdsBoundary
{
...
auto appendLogicCellBoundaries(ClockTreeGdsLibrary& library, ClockTreeVisualizationLayerPolicy& layer_policy,
...
auto appendClockInstBoundaries(ClockTreeGdsLibrary& library, ClockTreeVisualizationLayerPolicy& layer_policy,
                               const ClockTreeVisualizationModel& model) -> void
{
...
auto appendInstBoundaries(ClockTreeGdsLibrary& library, ClockTreeVisualizationLayerPolicy& layer_policy,
                          const ClockTreeVisualizationModel& model) -> void
{
...
auto appendSegments(ClockTreeGdsLibrary& library, ClockTreeVisualizationLayerPolicy& layer_policy, const ClockTreeVisualizationModel& model,
                    ClockTreeViewMode view) -> void
{
  const bool flyline = view == ClockTreeViewMode::kFlyline;
  ...
  for (const auto& segment : segments) {
    ...
    library.paths.push_back(ClockTreeGdsPath{
        .key = key,
...
auto buildLibrary(const std::string& library_name, const std::string& structure_name, const ClockTreeVisualizationModel& model,
                  ClockTreeViewMode view, ClockTreeVisualizationLayerPolicy& layer_policy) -> ClockTreeGdsLibrary
{
  ClockTreeGdsLibrary library{
...
auto EmitClockTreeGdsVisualizations(const std::filesystem::path& visualization_dir, const ClockTreeView& clock_tree_view)
    -> ClockTreeGdsVisualizationResult
{
  ...
  const auto model = ClockTreeVisualizationModelBuilder::build(clock_tree_view);
  if (!model.has_clocks || (model.design_segments.empty() && model.flyline_segments.empty())) {
    ...
    return ClockTreeGdsVisualizationResult{.success = false};
  }

  ClockTreeVisualizationLayerPolicy design_layer_policy;
  auto design_library = buildLibrary("CTS_DESIGN", "CTS_DESIGN", model, ClockTreeViewMode::kDesign, design_layer_policy);
  ClockTreeVisualizationLayerPolicy flyline_layer_policy;
  auto flyline_library = buildLibrary("CTS_FLYLINE", "CTS_FLYLINE", model, ClockTreeViewMode::kFlyline, flyline_layer_policy);

  const bool design_success = ClockTreeGdsWriter::writeBinary(design_gds_path, design_library);
  const bool flyline_success = ClockTreeGdsWriter::writeBinary(flyline_gds_path, flyline_library);
  const bool design_lyp_success = ClockTreeGdsWriter::writeLayerProperties(design_lyp_path, design_layer_policy.getLayerProperties());
  const bool flyline_lyp_success = ClockTreeGdsWriter::writeLayerProperties(flyline_lyp_path, flyline_layer_policy.getLayerProperties());
  emitReportTable(design_gds_path, design_success, design_lyp_path, design_lyp_success, flyline_gds_path, flyline_success, flyline_lyp_path,
                  flyline_lyp_success);
  return ClockTreeGdsVisualizationResult{.success = design_success && flyline_success && design_lyp_success && flyline_lyp_success};
}
