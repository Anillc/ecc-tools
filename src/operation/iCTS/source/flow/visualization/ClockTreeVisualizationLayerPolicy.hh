...
class ClockTreeVisualizationLayerPolicy
{
 public:
  ...
  auto logicCellLayer() -> ClockTreeGdsLayerKey;
  auto instLayer(const ClockTreeVisualizationInst& inst) -> std::optional<ClockTreeGdsLayerKey>;
  auto segmentLayer(const ClockTreeVisualizationSegment& segment) -> ClockTreeGdsLayerKey;
  auto getLayerProperties() const -> std::vector<ClockTreeGdsLayerProperty>;
  ...
  struct LayerKey
  {
    ...
    ClockTreeViewMode view = ClockTreeViewMode::kUnknown;
 ...
 private:
  auto getLayer(const LayerDescriptor& descriptor) -> ClockTreeGdsLayerKey;
