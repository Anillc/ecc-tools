...
std::string makeSteinerName(int id)
{
  return "steiner_" + std::to_string(id);
}

std::vector<icts::Pin*> buildSaltPinOrder(icts::Pin* driver, const std::vector<icts::Pin*>& loads)
{
...
std::vector<std::shared_ptr<salt::Pin>> buildSaltPins(const std::vector<icts::Pin*>& pins,
                                                      std::unordered_map<int, icts::Node*>* id_to_node = nullptr)
{
  std::vector<std::shared_ptr<salt::Pin>> salt_pins;
  salt_pins.reserve(pins.size());
  for (size_t i = 0; i < pins.size(); ++i) {
    auto* pin = pins[i];
    auto salt_pin = std::make_shared<salt::Pin>(pin->get_location().x(), pin->get_location().y(), i, pin->get_cap_load());
    if (id_to_node != nullptr) {
      (*id_to_node)[i] = pin;
    }
...
salt::Tree buildFluteSaltTree(const std::string& net_name, icts::Pin* driver, const std::vector<icts::Pin*>& loads,
                              std::unordered_map<int, icts::Node*>* id_to_node = nullptr)
{
  auto pins = buildSaltPinOrder(driver, loads);
  auto salt_pins = buildSaltPins(pins, id_to_node);
...
{
  return GetCtsRuntime().getConfig()->get_work_dir() + "/file";
}
...
double TreeBuilder::estimateFluteWireLength(Pin* driver, const std::vector<Pin*>& loads)
{
  if (driver == nullptr || loads.empty()) {
    return 0.0;
  }

  auto tree = buildFluteSaltTree("estimate_flute_wirelength", driver, loads);
  ...
  salt::TreeNode::preOrder(source, [&](const std::shared_ptr<salt::TreeNode>& salt_node) {
    if (salt_node == nullptr || salt_node->parent == nullptr) {
      return;
    }

    const Point parent_loc(salt_node->parent->loc.x, salt_node->parent->loc.y);
    const Point node_loc(salt_node->loc.x, salt_node->loc.y);
    total_wire_length += TimingPropagator::calcLen(parent_loc, node_loc);
  });
...
void TreeBuilder::fluteTree(const std::string& net_name, Pin* driver, const std::vector<Pin*>& loads)
{
  ...
  auto tree = buildFluteSaltTree(net_name, driver, loads, &id_to_node);
  ...
  auto connect_node_func = [&](const std::shared_ptr<salt::TreeNode>& salt_node) {
    ...
    if (salt_node->id > static_cast<int>(loads.size())) {
      auto name = makeSteinerName(salt_node->id);
...
/**
 * @brief Build a BST-SALT refinement tree from the current sink set.
 *
 * @param net_name
 * @param loads
 * @param skew_bound
...
/**
 * @brief Build the default shallow-light tree without additional refinement.
 *
 * @param net_name
 * @param loads
 * @param skew_bound
...
void TreeBuilder::convertToBinaryTree(Node* root)
{
  ...
  auto three_child_refine = [&](Node* node) {
    ...
    // Pair the nearest children around the current branching point as a local refinement heuristic.
...
void TreeBuilder::updateId(Node* root)
{
  ...
  std::ranges::for_each(steiner_nodes, [&](Node* node) {
    node->set_id(id++);
    node->set_name(makeSteinerName(node->get_id()));
  });
}
/**
