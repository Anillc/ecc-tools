...
class TreeBuilder
{
 public:
  ...
  static double estimateFluteWireLength(Pin* driver, const std::vector<Pin*>& loads);
  ...
  static Inst* defaultTree(const std::string& net_name, const std::vector<Pin*>& loads,
                           const std::optional<double>& skew_bound = std::nullopt, const std::optional<Point>& guide_loc = std::nullopt,
                           const TopoType& topo_type = TopoType::kGreedyDist);
