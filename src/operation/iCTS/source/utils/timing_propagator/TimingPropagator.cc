...
void TimingPropagator::init()
{
  ...
  _unit_cap = GetCtsRuntime().getClockUnitCap();
  _unit_res = GetCtsRuntime().getClockUnitRes() / 1000;
  _unit_h_cap = GetCtsRuntime().getClockUnitCap(LayerPattern::kH);
  _unit_h_res = GetCtsRuntime().getClockUnitRes(LayerPattern::kH) / 1000;
  _unit_v_cap = GetCtsRuntime().getClockUnitCap(LayerPattern::kV);
  _unit_v_res = GetCtsRuntime().getClockUnitRes(LayerPattern::kV) / 1000;
  _db_unit = GetCtsRuntime().getDbUnit();
  _delay_libs = GetCtsRuntime().getAllBufferLibs();
...
void TimingPropagator::updatePinCap(Pin* pin)
{
  double cap_load = 0;
  if (pin->isSinkPin()) {
    cap_load = GetCtsRuntime().getSinkCap(pin->get_name());
  }
  if (pin->isBufferPin()) {
    ...
    auto* lib = GetCtsRuntime().getCellLib(cell_name);
