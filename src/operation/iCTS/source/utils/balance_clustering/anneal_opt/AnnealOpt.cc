...
std::vector<std::vector<Pin*>> AnnealOptInterface::run(const bool& log)
{
  initCostMap();
  if (_auto_temp) {
    // Scale the initial temperature with the current solution size for stable annealing startup.
    _temperature = _cur_cost / std::ceil(1.0 * _cur_solution.size() / 20);
  }
  ...
  if (_improve > 0) {
    ...
    oss << "Best Cost: " << std::fixed << std::setprecision(3) << _best_cost << "  Improvement: " << _improve * 100 << "%";
    GetCtsRuntime().saveToLog(oss.str());
  } else {
