                                         ...
                                         BuildProgress& build_progress) -> void
{
  ...
  for (const double load_pf : _loads_to_test) {
    ...
    if (!buf_masters.empty()) {
      const auto* first_buffer = findCharacterizationBufferCell(buf_masters.front());
      ...
      driven_cap_pf += calcClockRouteWireCapPf(topo.wire_segments_um.front());
    } else {
      driven_cap_pf = load_pf;
      for (const double seg_len_um : topo.wire_segments_um) {
        driven_cap_pf += calcClockRouteWireCapPf(seg_len_um);
      }
    }
