...
auto CharBuilder::findCharacterizationBufferCell(const std::string& cell_master) const -> const CharacterizationBufferCell*
{
  auto it = std::ranges::find_if(_sorted_buffers, [&cell_master](const CharacterizationBufferCell& buffer_cell) -> bool {
    return buffer_cell.cell_master == cell_master;
  });
  return it == _sorted_buffers.end() ? nullptr : &(*it);
}

auto CharBuilder::calcClockRouteWireCapPf(double wirelength_um) const -> double
{
  return std::max(0.0, wirelength_um) * _clock_route_segment_rc.capacitance_per_um_pf;
}
    ...
    -> PatternFeasibility
{
  ...
  for (std::size_t segment_index = 0; segment_index < topo.wire_segments_um.size(); ++segment_index) {
    const CharacterizationBufferCell* driver_buffer = nullptr;
    if (segment_index == 0U) {
    ...
    const CharacterizationBufferCell* next_buffer
