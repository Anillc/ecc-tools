...
auto resolveBufferDriveCap(const CharacterizationBufferCell& buffer_cell) -> double
{
...
auto resolveClockRouteSegmentRc(const CharBuilder::InitOptions& options) -> ClockRouteSegmentRc
{
...
auto collectSortedBuffers(const CharBuilder::InitOptions& options) -> std::vector<CharacterizationBufferCell>
{
  std::vector<CharacterizationBufferCell> sorted_buffers;
  ...
  std::ranges::sort(sorted_buffers,
                    [](const CharacterizationBufferCell& lhs_buffer_cell, const CharacterizationBufferCell& rhs_buffer_cell) -> bool {
  ...
  if (buffer_redundancy_pct > 0.0 && sorted_buffers.size() > 1U) {
    std::vector<CharacterizationBufferCell> filtered_buffers;
...
auto resolveWirelengthUnitUm(const std::vector<CharacterizationBufferCell>& sorted_buffers) -> ResolvedValue
{
...
auto resolveWirelengthUnitUm(const CharBuilder::InitOptions& options, const std::vector<CharacterizationBufferCell>& sorted_buffers,
...
auto CharBuilder::init(const InitOptions& options) -> void
{
  ...
  _clock_route_segment_rc = clock_route_segment_rc;
