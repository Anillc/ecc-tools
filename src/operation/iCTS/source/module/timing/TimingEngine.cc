...
auto TimingEngine::updateDownstreamCap(RCTree& rc_tree) -> void
{
  ...
  for (const auto vertex_id : std::ranges::reverse_view(preorder)) {
    ...
    for (auto arc_id : vertex->child_arc_ids) {
      ...
      CTS_LOG_FATAL_IF(arc == nullptr) << "RCTree arc is null during downstream capacitance update.";

      const auto* child = rc_tree.get_vertex(arc->sink_vertex_id);
      CTS_LOG_FATAL_IF(child == nullptr) << "RCTree child vertex is null during downstream capacitance update.";
...
auto TimingEngine::updateIncreaseDelay(RCTree& rc_tree) -> void
{
  for (auto& arc : rc_tree.get_arcs()) {
    const auto* child = rc_tree.get_vertex(arc.sink_vertex_id);
    CTS_LOG_FATAL_IF(child == nullptr) << "RCTree child vertex is null during arc delay update.";
...
auto TimingEngine::updateArrival(RCTree& rc_tree) -> void
{
  ...
  CTS_LOG_FATAL_IF(root == nullptr) << "RCTree root vertex is null during arrival update.";
  root->arrival = 0.0;

  for (auto vertex_id : preorder) {
    const auto* vertex = rc_tree.get_vertex(vertex_id);
    CTS_LOG_FATAL_IF(vertex == nullptr) << "RCTree vertex is null during arrival update.";

    for (auto arc_id : vertex->child_arc_ids) {
      const auto* arc = rc_tree.get_arc(arc_id);
      CTS_LOG_FATAL_IF(arc == nullptr) << "RCTree arc is null during arrival update.";

      auto* child = rc_tree.get_vertex(arc->sink_vertex_id);
      CTS_LOG_FATAL_IF(child == nullptr) << "RCTree child vertex is null during arrival update.";
...
auto TimingEngine::updateSlew(RCTree& rc_tree) -> void
{
  ...
  auto* root = rc_tree.get_vertex(preorder.front());
  CTS_LOG_FATAL_IF(root == nullptr) << "RCTree root vertex is null during slew update.";
  root->slew = 0.0;

  for (auto vertex_id : preorder) {
    const auto* vertex = rc_tree.get_vertex(vertex_id);
    CTS_LOG_FATAL_IF(vertex == nullptr) << "RCTree vertex is null during slew update.";

    for (auto arc_id : vertex->child_arc_ids) {
      const auto* arc = rc_tree.get_arc(arc_id);
      CTS_LOG_FATAL_IF(arc == nullptr) << "RCTree arc is null during slew update.";
...
auto TimingEngine::updateDownstreamDelay(RCTree& rc_tree) -> void
{
  ...
  for (const auto vertex_id : std::ranges::reverse_view(preorder)) {
    ...
    for (auto arc_id : vertex->child_arc_ids) {
      const auto* arc = rc_tree.get_arc(arc_id);
      CTS_LOG_FATAL_IF(arc == nullptr) << "RCTree arc is null during downstream delay update.";

      const auto* child = rc_tree.get_vertex(arc->sink_vertex_id);
      CTS_LOG_FATAL_IF(child == nullptr) << "RCTree child vertex is null during downstream delay update.";
...
auto TimingEngine::evaluate(const RCTree& rc_tree) -> TimingEngine::Metrics
{
  ...
  CTS_LOG_FATAL_IF(root == nullptr) << "RCTree root vertex is null during timing evaluation.";
...
auto TimingEngine::calcSkew(const RCTree& rc_tree) -> double
{
  ...
  const auto* root = rc_tree.get_vertex(rc_tree.get_root());
  CTS_LOG_FATAL_IF(root == nullptr) << "RCTree root vertex is null during skew calculation.";
