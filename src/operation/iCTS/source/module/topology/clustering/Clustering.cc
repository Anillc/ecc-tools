...
std::vector<Point<double>> calcCenters(const std::vector<std::vector<Pin*>>& clusters)
{
  ...
  for (const auto& cluster : clusters) {
    centers.push_back(geometry::calc_center(cluster, [](Pin* pin) { return pin->get_location(); }));
  }
  return centers;
}
...
ClusterResult Clustering::biPartition(const std::vector<Pin*>& loads, std::size_t min_cluster_size) const
{
  ...
  for (int iter = 0; iter < _config.max_iter; ++iter) {
    ...
    for (auto* pin : loads) {
      ...
      mcf.add_node(static_cast<double>(loc.x()), static_cast<double>(loc.y()), pin);
    }
    for (const auto& center : centers) {
      mcf.add_center(center.x(), center.y());
    }
  ...
  for (const auto& center : centers) {
    result.centers.emplace_back(static_cast<int>(std::lround(center.x())), static_cast<int>(std::lround(center.y())));
  }
