...
auto FLUTERouter::buildTree(const Terminal& driver_terminal, const std::vector<Terminal>& load_terminals) -> FLUTERouter::SteinerTreeType
{
  ...
  auto connect_node_func = [&](const std::shared_ptr<salt::TreeNode>& salt_node) -> void {
    ...
    CTS_LOG_FATAL_IF(current_node == nullptr || parent_node == nullptr) << "FLUTE routing tree node is null.";

    auto edge_id = steiner_tree.addEdge(parent_id, current_id, geometry::Manhattan(parent_node->location, current_node->location));
    CTS_LOG_FATAL_IF(edge_id == SteinerTreeType::kInvalidId) << "Failed to add edge when building FLUTE SteinerTree.";
  };
  salt::TreeNode::preOrder(source, connect_node_func);

  CTS_LOG_FATAL_IF(!steiner_tree.validate()) << "Constructed FLUTE SteinerTree is invalid.";
  return steiner_tree;
}
