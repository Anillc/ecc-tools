 ...
 *       Propagate routing timing information from root to leaf.
 *       Scope is limited to wire-related timing data: wirelength, capacitance,
 *       slew and wire delay.
 *
 */
class TimingPropagator
{
 public:
  ...
  // get
  static double getSkewBound();
  static int getDbUnit();
  static double getMaxBufTran();
  static double getMaxSinkTran();
  static double getMaxCap();
  static int getMaxFanout();
   ...
   *       driver pin: use zero source slew in stage-1 routing migration.
   *       load pin or steiner node: slew_in = sqrt([parent's slew_in]^2 + [slew_wire]^2)
   *
   * @tparam T
   * @param node
   * @param pattern
  ...
  {
    auto calc_slew_in = [&node, &pattern](T* child) {
      auto slew_ideal = calcIdealSlew(node, child, pattern);
  ...
  {
    return 1.0 * calcDist(p1, p2, pattern) / getDbUnit();
  }
