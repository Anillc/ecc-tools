...
Inst* BSTRouter::route(const std::string& net_name, const std::vector<Pin*>& load_pins, const std::optional<double>& skew_bound,
                       const std::optional<Point>& guide_loc, const TopoType& topo_type)
{
  return TreeBuilder::boundSkewTree(net_name, load_pins, skew_bound, guide_loc, topo_type);
}
