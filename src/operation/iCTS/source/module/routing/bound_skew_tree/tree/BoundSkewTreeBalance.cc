...
auto BoundSkewTree::calcBalancePointOnLine(const BalancePointQuery& query, BalancePointResult& result) const -> void
{
  ...
  if (Equal(horizontal_distance, 0)) {
    first_delay_increase = calcDelayIncrease(0, result.distance_to_first, query.first_point.val, query.rc_pattern);
    second_delay_increase = calcDelayIncrease(0, result.distance_to_second, query.second_point.val, query.rc_pattern);
  } else {
    first_delay_increase = calcDelayIncrease(result.distance_to_first, 0, query.first_point.val, query.rc_pattern);
    second_delay_increase = calcDelayIncrease(result.distance_to_second, 0, query.second_point.val, query.rc_pattern);
  }
...
auto BoundSkewTree::calcBalancePointOffLine(const BalancePointQuery& query, BalancePointResult& result) const -> void
{
  ...
  if (x_position < 0) {
    ...
    auto delay_increase = calcDelayIncrease(horizontal_distance, vertical_distance, second_point.val, query.rc_pattern);
    ...
    auto new_delay_increase = calcDelayIncrease(0, result.distance_to_second, adjusted_point.val, query.rc_pattern);
    ...
    auto delay_increase = calcDelayIncrease(horizontal_distance, vertical_distance, first_point.val, query.rc_pattern);
    ...
    auto new_delay_increase = calcDelayIncrease(0, result.distance_to_first, adjusted_point.val, query.rc_pattern);
    ...
    auto first_delay_increase = calcDelayIncrease(x_position, y_position, first_point.val, query.rc_pattern);
    auto second_delay_increase
        = calcDelayIncrease(horizontal_distance - x_position, vertical_distance - y_position, second_point.val, query.rc_pattern);
...
{
  auto resistance_capacitance_cross_term = _rc_pattern == BSTRoutingRCPattern::kHV
                                               ? _unit_vertical_resistance * _unit_horizontal_capacitance
...
auto BoundSkewTree::addMergeRegionPointFromJoiningRegion(Area* current_area, const size_t& side, const size_t& point_index) const -> void
{
  ...
  if (dist <= 0) {
    ...
    const auto incr_delay
        = side == kLeft ? calcDelayIncrease(horizontal_distance, vertical_distance, current_area->get_left()->get_cap_load(), _rc_pattern)
                        : calcDelayIncrease(horizontal_distance, vertical_distance, current_area->get_right()->get_cap_load(), _rc_pattern);
