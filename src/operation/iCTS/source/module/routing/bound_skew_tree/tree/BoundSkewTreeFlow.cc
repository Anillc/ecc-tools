...
auto BoundSkewTree::updateEmbeddedNodeTiming(Area* current) const -> void
{
  ...
  const auto delay_to_left = pointDelayIncrease(parent_point, left_point, current->get_edge_len(kLeft), left->get_cap_load(), _rc_pattern);
  const auto delay_to_right
      = pointDelayIncrease(parent_point, right_point, current->get_edge_len(kRight), right->get_cap_load(), _rc_pattern);
