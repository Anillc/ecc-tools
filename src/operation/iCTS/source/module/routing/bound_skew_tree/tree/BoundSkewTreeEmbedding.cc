...
// ***************************************************************************************
/**
 * @file BoundSkewTreeEmbedding.cc
 * @author Dawn Li (dawnli619215645@gmail.com)
 * @date 2026-04-24
 * @brief Bound-skew tree embedding and point-delay updates.
 */
...
auto BoundSkewTree::updatePointDelaysByEndSide(const Area& current_area, const size_t& end_side, Point& point) const -> void
{
  ...
  const auto delay_left = pointDelayIncrease(point, left_endpoint, left_child->get_cap_load(), _rc_pattern);
  const auto delay_right = pointDelayIncrease(point, right_endpoint, right_child->get_cap_load(), _rc_pattern);
...
auto BoundSkewTree::calcIrregularPointDelays(const Area& current_area, Point& point, Line& boundary_segment) const -> void
{
  ...
  if (joining_segment_type == LineType::kManhattan) {
    ...
    auto delay_left = pointDelayIncrease(left_line.at(kHead), point, left_child->get_cap_load(), _rc_pattern);
    auto delay_right = pointDelayIncrease(right_line.at(kHead), point, right_child->get_cap_load(), _rc_pattern);
  ...
  if (Equal(horizontal_distance, 0)) {
    ...
    if (length > horizontal_distance + vertical_distance) {
      delay += calcDelayIncrease(
          ...
          cap_load + (_unit_horizontal_capacitance * horizontal_distance) + (_unit_vertical_capacitance * vertical_distance), rc_pattern);
    }
  }
  LOG_FATAL_IF(delay < 0) << "point increase delay is negative";
  return delay;
