...
auto SALTRouter::buildTree(const ClockTerminal& driver_terminal, const std::vector<ClockTerminal>& load_terminals)
    -> SALTRouter::ClockSteinerTreeType
{
  ...
  auto connect_node_func = [&](const std::shared_ptr<salt::TreeNode>& salt_node) -> void {
    ...
    CTS_LOG_FATAL_IF(current_node == nullptr || parent_node == nullptr) << "SALT clock routing tree node is null.";

    const auto distance = geometry::Manhattan(parent_node->location, current_node->location);
    CTS_LOG_FATAL_IF(distance < 0) << "SALT embedded edge distance is negative.";
    auto edge_id = clock_tree.addEdge(parent_id, current_id, distance, distance);
    CTS_LOG_FATAL_IF(edge_id == ClockSteinerTreeType::kInvalidId) << "Failed to add edge when building SALT ClockSteinerTree.";
  };
  salt::TreeNode::preOrder(source, connect_node_func);

  CTS_LOG_FATAL_IF(!clock_tree.validate()) << "Constructed SALT ClockSteinerTree is invalid.";
  return clock_tree;
}
