...
void LocalLegalization::legalize()
{
  ...
  auto within_core = [](const Point& point) { return CTSAPIInst.withinCore(point.x(), point.y()); };

  std::set<Point> set;
  std::ranges::for_each(_fixed_locations, [&](const Point& loc) {
    if (set.contains(loc)) {
      LOG_FATAL << "Fixed locations are not legal";
    }
    set.insert(loc);
  });
  ...
  for (size_t i = 0; i < _variable_locations.size(); ++i) {
    ...
    while (!legal && step < max_step) {
      for (auto& dir : derection) {
        ...
        if (!set.contains(new_loc) && (_ignore_core || within_core(new_loc))) {
          set.insert(new_loc);
    ...
    if (!legal) {
      LOG_FATAL << "Can not legalize the location";
    }
  }
}
}  // namespace icts