...
void LibLibrary::printLibertyLibrary(const char* lib_file_name)
{
  ...
  auto wirte_axis = [&stream](LibAxis* lib_axis, int index_order) {
    ...
    for (int i = 0; i < axis_values.size(); ++i) {
      auto axis_float_value = dynamic_cast<LibFloatValue*>(axis_values[i].get())->getFloatValue();
  ...
  auto wirte_table_values = [&stream](LibTable* lib_table, int columns) {
    ...
    for (int i = 0; i < lib_table_values.size(); ++i) {
      auto lib_table_float_value = dynamic_cast<LibFloatValue*>(lib_table_values[i].get())->getFloatValue();
  ...
  auto wirte_delay_table_model = [&stream, &wirte_axis, &wirte_table_values](LibArc* lib_arc) {
    std::map<LibArc::TimingSense, const char*> timing_sense_map = {{LibArc::TimingSense::kPositiveUnate, "positive_unate"},
    ...
    if (cell_fall_table) {
      ...
      if (lut_table_template) {
        ...
        auto float_value = dynamic_cast<LibFloatValue*>(cell_fall_table_values.front().get())->getFloatValue();
    ...
    if (cell_rise_table) {
      auto* lut_table_template1 = cell_rise_table->get_table_template();
      if (lut_table_template1) {
        ...
        wirte_table_values(cell_rise_table, columns);
        fprintf(stream, "                       }\n");
      } else {
        auto& cell_rise_table_values = cell_rise_table->get_table_values();
        LOG_FATAL_IF(cell_rise_table_values.size() > 1);
        auto float_value = dynamic_cast<LibFloatValue*>(cell_rise_table_values.front().get())->getFloatValue();
    ...
    if (fall_transition_table) {
      ...
      if (lut_table_template2) {
        ...
        // auto& fall_transition_table_values =
        // fall_transition_table->get_table_values();
        ...
        auto float_value = dynamic_cast<LibFloatValue*>(fall_transition_table_values.front().get())->getFloatValue();
    ...
    if (rise_transition_table) {
      ...
      if (lut_table_template3) {
        ...
        // auto& rise_transition_table_values =
        // rise_transition_table->get_table_values();
        ...
        auto float_value = dynamic_cast<LibFloatValue*>(rise_transition_table_values.front().get())->getFloatValue();
  ...
  auto wirte_check_table_model = [&stream, &wirte_axis, &wirte_table_values](LibArc* lib_arc) {
    std::map<LibArc::TimingType, const char*> timing_type_map = {{LibArc::TimingType::kSetupRising, "setup_rising"},
    ...
    LibTableModel* table_model = lib_arc->get_table_model();
    LibCheckTableModel* check_model = dynamic_cast<LibCheckTableModel*>(table_model);

    // fall_constraint table
    LibTable* fall_constraint_table
        = dynamic_cast<LibCheckTableModel*>(table_model)->getTable(int(LibTable::TableType::kFallConstrain) - 4);
    ...
    if (lut_table_template) {
      ...
      auto float_value = dynamic_cast<LibFloatValue*>(fall_constraint_table_values.front().get())->getFloatValue();
    ...
    LibTable* rise_constraint_table
        = dynamic_cast<LibCheckTableModel*>(table_model)->getTable(int(LibTable::TableType::kRiseConstrain) - 4);
    ...
    if (lut_table_template1) {
      ...
      // auto& rise_constraint_table_values =
      // rise_constraint_table->get_table_values();
      ...
      auto float_value = dynamic_cast<LibFloatValue*>(rise_constraint_table_values.front().get())->getFloatValue();
  ...
  auto write_liberty_cell
      = [&stream, &classify_cell_arc_by_snk_port, &wirte_delay_table_model, &wirte_check_table_model](LibCell* lib_cell) {
